...
public class EpisodeController {
    ...
    public EpisodeCreateResponseDto postEpisode(
        final @PathVariable Long videoId, final @Validated @RequestBody EpisodeCreateDto dto
    ) {
        // Pass videoId to the service method
        EpisodeCreateRequestDto createDto = new EpisodeCreateRequestDto(videoId, dto);
    ...
    public ResponseDto<EpisodeReadResponseDto> getEpisode(
        final @PathVariable Long videoId, final @PathVariable Long episodeId
    ) {
    ...
    public ResponseEntity<Object> watchEpisode(
        ...
        if (!canAccess) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("이 에피소드를 볼려면 맴버쉽 등급을 높이세요.");
        }
